...
!! @param[in] size_gas Length of the string gas.
!! @param[in] size_solvent Length of the string gas.
!! @param[out] kh Henry constante in mole fraction per GPa. NaN if gas not found.
subroutine iapws_capi_kh_vs_T(T, gas, solvent, kh, size_T, size_gas, size_solvent)bind(C)
    implicit none
    !! arguments
    real(c_double), dimension(size_T), intent(in) :: T
    type(c_ptr), intent(in), value :: gas
    type(c_ptr), intent(in), value :: solvent 
    integer(c_int), intent(in), value :: size_T
    integer(c_int), intent(in), value :: size_gas
    integer(c_int), intent(in), value :: size_solvent
    !! returns
    real(c_double), dimension(size_T), intent(out) :: kh
    ...
    call iapws_kh_vs_t(T, f_gas, f_solvent, kh)
